    ...
    "db-migration": BlastRadiusRequest(
        ...
        teams=[
            Team(
                id="platform",
                name="Platform Team",
                owns=["auth-service", "session-manager"],
                size=6,
                focus="platform",
            ),
            Team(
            ...
            Team(
                ...
                owns=["api-gateway", "notification-service"],
                size=5,
                focus="platform",
            ),
        ],
    ...
    "api-deprecation": BlastRadiusRequest(
        services=[
            Service(
                ...
                owner_team="payments",
                criticality="critical",
                description="Legacy payments API serving checkout flows and 3 external partner integrations. REST API with XML response format.",
            ),
            Service(
                id="payments-v2",
                ...
                owner_team="payments",
                criticality="critical",
                description="New payments API with JSON responses, webhook support, and improved retry logic. Target migration destination.",
            ),
            Service(
                id="checkout-service",
                ...
                owner_team="growth",
                criticality="critical",
                description="Core checkout flow. Currently calls payments-v1. Migration to v2 is in progress but not complete.",
            ),
            Service(
                id="billing-service",
                ...
                owner_team="payments",
                criticality="high",
                description="Handles subscription billing, invoicing, and payment method management. Uses payments-v1 for recurring charges.",
            ),
            Service(
                ...
                owner_team="data",
                criticality="medium",
                description="Ingests payment events from v1 API for revenue reporting. Has custom parsing logic for v1 response format.",
            ),
            Service(
                ...
                owner_team="growth",
                criticality="high",
                description="Backend for iOS and Android apps. Calls payments-v1 for in-app purchases. Mobile release cycle is 2 weeks.",
            ),
            Service(
                id="partner-integration-layer",
                name="Partner Integration Layer",
                owner_team="partner-eng",
                criticality="high",
                description="Manages 3 external partner integrations (Acme Corp, Beta Inc, Gamma Ltd) all consuming payments-v1 via XML API.",
            ),
        ],
        dependencies=[
            Dependency(
                from_service="checkout-service",
                to_service="payments-v1",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
                from_service="checkout-service",
                to_service="payments-v2",
                dependency_type="sync-api",
                strength="soft",
            ),
            Dependency(
                from_service="billing-service",
                to_service="payments-v1",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
                from_service="analytics-pipeline",
                to_service="payments-v1",
                dependency_type="async-event",
                strength="soft",
            ),
            Dependency(
                from_service="mobile-app-backend",
                to_service="payments-v1",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
                from_service="partner-integration-layer",
                to_service="payments-v1",
                dependency_type="sync-api",
                strength="hard",
            ),
        ...
        teams=[
            Team(
                id="payments",
                name="Payments Team",
                owns=["payments-v1", "payments-v2", "billing-service"],
                size=7,
                focus="product",
            ),
            Team(
                id="growth",
                name="Growth Team",
                owns=["checkout-service", "mobile-app-backend"],
                size=9,
                focus="product",
            ),
            Team(
            ...
            Team(
                ...
                owns=["partner-integration-layer"],
                size=4,
                focus="product",
            ),
        ],
    ...
    "platform-reorg": BlastRadiusRequest(
        services=[
            Service(
                ...
                owner_team="platform",
                criticality="critical",
                description="Internal CI/CD tooling, pipeline infrastructure, and developer tooling. Transitioning to DevEx team.",
            ),
            Service(
                id="k8s-cluster-mgmt",
                ...
                owner_team="platform",
                criticality="critical",
                description="Cluster provisioning, scaling, and node management. Transitioning to Infra team.",
            ),
            Service(
                ...
                owner_team="platform",
                criticality="critical",
                description="Vault integration, secret rotation, and access control. Ownership TBD between Infra and Security.",
            ),
            Service(
                id="service-mesh",
                ...
                owner_team="platform",
                criticality="high",
                description="mTLS, traffic management, and observability. Transitioning to Infra team.",
            ),
            Service(
                ...
                owner_team="platform",
                criticality="medium",
                description="Internal tooling docs and service catalog. Transitioning to DevEx team.",
            ),
            Service(
                id="monitoring-stack",
                ...
                owner_team="platform",
                criticality="high",
                description="Prometheus, Grafana, alerting rules. Joint ownership during transition.",
            ),
            Service(
                id="auth-service",
                name="Authentication Service",
                owner_team="product",
                criticality="critical",
                description="Depends on secrets-management and service-mesh. Ownership unchanged but affected by platform reorg.",
            ),
            Service(
                id="payments-core",
                name="Payments Core",
                owner_team="product",
                criticality="critical",
                description="Depends on k8s-cluster-mgmt for deployment and secrets-management for credentials.",
            ),
        ],
        dependencies=[
            Dependency(
                from_service="auth-service",
                to_service="secrets-management",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
                from_service="auth-service",
                to_service="service-mesh",
                dependency_type="shared-library",
                strength="hard",
            ),
            Dependency(
                from_service="payments-core",
                to_service="k8s-cluster-mgmt",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
            ...
            Dependency(
                from_service="ci-cd-platform",
                to_service="k8s-cluster-mgmt",
                dependency_type="sync-api",
                strength="hard",
            ),
            Dependency(
                from_service="monitoring-stack",
                to_service="k8s-cluster-mgmt",
                dependency_type="sync-api",
                strength="soft",
            ),
        ...
        teams=[
            ...
            Team(
                ...
                owns=["auth-service", "payments-core"],
                size=20,
                focus="product",
            ),
            Team(
...
EXAMPLE_METADATA = [
    {
        ...
        "description": "Migrating user auth database from PostgreSQL to Aurora",
    },
    {
        "id": "api-deprecation",
        "title": "API Deprecation",
        "description": "Deprecating v1 Payments API — 90-day sunset",
    },
    {
        "id": "platform-reorg",
        "title": "Platform Team Reorg",
        "description": "Splitting Platform team into Infrastructure and Developer Experience sub-teams",
    },
]
